...
> 原文(中国語wiki等)を渡して「`<W>-<M>` を更新して」と依頼された場合は、ロジックだけでなく原文txt・`branch.ts` も含めた3点セットの更新が必要。まず `branch-update` スキルを参照すること。本スキルはそのうちロジック実装部分を担当する。
...
2. **未指定組合せ**: 表に指定されていない組み合わせに気付いたら、即座に作業を停止してユーザーに確認する。推測で補完しない。実行時の取りこぼしは末尾の `omission_of_conditions` が受ける。
   - **例外(索敵値のみの分岐)**: その地点の分岐条件が索敵値のみで、かつ具体的な要求索敵値・分岐点係数が記述されていない場合は、確認せず「要求索敵値不明につき、全て〇」として索敵充足側の遷移先1つに固定する(例 `case 'Q': return 'R';`)。
